...
class TestPositive:
    ...
    def test_2(self, clear_folders, make_files, save_stat):
        ...
        res = []
        res.append(checkout(f"cd {data['folder_tst']}; 7z a {data['folder_out']}/{data['output_file']}",
                            "Everything is Ok"))
        res.append(checkout(f"cd {data['folder_out']}; 7z e {data['output_file']} -o{data['folder_ext']}",
                            "Everything is Ok"))
    ...
    def test_7(self, clear_folders, make_files, make_sub_folders, save_stat):
        # test7 распаковка с сохранением структуры
        res = []
        res.append(checkout(f"cd {data['folder_tst']}; 7z a {data['folder_out']}/{data['output_file']}",
                            "Everything is Ok"))
        res.append(checkout(f"cd {data['folder_out']}; 7z x {data['output_file']} -o{data['folder_ext']}",
                            "Everything is Ok"))
        for file_name in make_files:
            res.append(checkout(f"ls {data['folder_ext']}; ", file_name))
